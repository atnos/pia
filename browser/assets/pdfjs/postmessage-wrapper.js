(function() {
  ...
  console.log('🟢 postmessage-wrapper.js: TEST LOG - BUILD_ID:', '2025-07-11T12-41-25-000Z');
  ...
  function transformZoomFromViewer(scale, app) {
    // If scale is already a string, it might be a special zoom mode
    if (typeof scale === 'string') {
      return scale;
    }
    
    // Check if we're in a special zoom mode by examining the PDF viewer state
    if (app && app.pdfViewer) {
      const viewer = app.pdfViewer;
      
      // Check if we're in page-fit mode
      if (viewer.currentScaleValue === 'page-fit') {
        return 'page-fit';
      }
      
      // Check if we're in page-width mode
      if (viewer.currentScaleValue === 'page-width') {
        return 'page-width';
      }
      
      // Check if we're in page-actual mode
      if (viewer.currentScaleValue === 'page-actual') {
        return 'page-actual';
      }
      ...
      // Check if the scale matches one of our predefined zoom levels
      ...
      // Check if the scale is close to a predefined zoom level (within 1%)
    ...
    // If we can't determine the special mode, fall back to percentage
  ...
  // Set up bidirectional event listeners for PDF.js state changes
